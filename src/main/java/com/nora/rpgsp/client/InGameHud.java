		...
		if (playerEntity != null) {
			...
			if (skillpowerInterface.getMaxSkillpower() >= 1 && skillpowerInterface.getTimeFull() < 60 && !(playerEntity instanceof SpellCasterClient client && client.isCastingSpell())) {
				if(!Rpgsp.clientConfig.alt) {
					...
					drawContext.drawTexture(spback, (int) ((k / 2) +(Rpgsp.clientConfig.Spbar_x)), scaledHeight +(Rpgsp.clientConfig.Spbar_y), 0, 0, 182, 5, 182, 5);
					if (skillpowerprop > 0) {
						drawContext.drawTexture(spbar, (int) ((k / 2) +(Rpgsp.clientConfig.Spbar_x)), scaledHeight +(Rpgsp.clientConfig.Spbar_y), 0, 0, Math.min(i, 182), 5, 182, 5);
					} else {
						drawContext.drawTexture(spbarneg, (int) ((k / 2) +(Rpgsp.clientConfig.Spbar_x)), scaledHeight +(Rpgsp.clientConfig.Spbar_y), 0, 0, Math.min(-i, 182), 5, 182, 5);

					}
					drawContext.drawTexture(spticks, (int) ((k / 2) +(Rpgsp.clientConfig.Spbar_x)), scaledHeight +(Rpgsp.clientConfig.Spbar_y), 0, 0, Math.min(i, 182), 5, 182, 5);
