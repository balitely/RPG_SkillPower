...
public class SpellTooltipMixin {
    private static float getSkillpowerCost(PlayerEntity player,Spell spell, ImpactContext context, float coeff, int proj){
    ...
    private static List<Text> spellInfoSkillpower(List<Text> tooltip, Identifier spellId, PlayerEntity player, ItemStack itemStack, boolean details) {
        try {
            ...
            if (!matches(spellId.toString(), Rpgsp.config.blacklist_spell_casting_regex) &&!(needsArrow) && spell.release != null && spell.cost != null && spell.cost.item_id != null) {
                ...
                MutableText text = Text.literal(" ").append(Text.translatable("rpgsp.skillpowercost").append(Text.literal(": ").append(String.valueOf((int) Math.round(getSkillpowerCost(player, spell, context, coeff, proj))))));
                if (SpellRegistry.getSpell(spellId).cast.channel_ticks > 0) {
                    text.append(Text.translatable("rpgsp.persecond"));
                }
